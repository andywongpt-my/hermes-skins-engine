...
def installed_skins() -> dict[str, Path]:
    ...
    if not d.exists():
        return {}
    return {f.stem: f for f in d.glob("*.yaml")}
...
@app.command()
def preview(name: str = typer.Argument(None, help="Skin name (installed or template). If omitted, shows the active skin.")):
    """Preview a skin in the terminal."""
    if name is None:
        # Show active skin from config
        typer.echo("No name given. Showing all templates. Use 'hermes-skins preview <name>' for a specific one.")
        for tname in THEMES:
    ...
    if name in skins:
        ...
        typer.echo(f"Installed: {', '.join(sorted(skins))}", err=True)
...
def custom(
    ...
    harmony: str = typer.Option("complementary", "--harmony", "-H", help="Color harmony: complementary, analogous, triadic, monochrome, split_comp"),
